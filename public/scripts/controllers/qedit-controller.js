app.controller('qeditCtr', ['$scope', '$state', '$timeout', '$document', 'ngDialog', 'bookmarkService', 'pubSubService', 'dataService', function($scope, $state, $timeout, $document, ngDialog, bookmarkService, pubSubService, dataService) {
    console.log("Hello qeditCtr");
    var maxSelections = 3;
    var dialog = null;
    ...
    $scope.ok = function() {
        ...
        if ($scope.add) {
            bookmarkService.addBookmark(params)
                .then((data) => {
                    $('.ui.modal.js-add-bookmark').modal('hide');
                    pubSubService.publish('QeditCtr.inserBookmarsSuccess', data);
                    if (data.title) {
